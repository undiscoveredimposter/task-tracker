---
description: Take the next area:frontend work item to a pull request
argument-hint: "[issue number]"
---

Run the `frontend-agent` subagent to take one frontend work item from the backlog through to
a pull request.

Invoke it with the Agent tool using `subagent_type: "frontend-agent"` and
**`run_in_background: false`**. Waiting matters: this command is designed to be run on a
loop, and a backgrounded agent would let the next tick start a second frontend agent while
...
`area:frontend` and `status:changes-requested` first — work coming back from review outranks
new work — then `area:frontend` + `status:ready`, oldest first. Anything `status:blocked` is
skipped until its dependency lands.
...
- what it verified by hand and at which viewport, for visual work
